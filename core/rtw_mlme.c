...
sint _rtw_init_mlme_priv(_adapter *padapter) {
    ...
    pmlmepriv->scan_mode =
        SCAN_ACTIVE; /* 1: active, 0: pasive. Maybe someday we should rename
...
int rtw_mlme_update_wfd_ie_data(struct mlme_priv *mlme, u8 type, u8 *ie,
    ...
    if (!hal_chk_wl_func(adapter, WL_FUNC_MIRACAST)) goto success;
    ...
    if (*t_ie && *t_ie_len) {
        ...
        attr_content = rtw_get_wfd_attr_content(
            *t_ie, *t_ie_len, WFD_ATTR_DEVICE_INFO, NULL, &attr_contentlen);
...
struct wlan_network *_rtw_alloc_network(
    struct mlme_priv *pmlmepriv) /* (_queue *free_queue) */
{
    _irqL irqL;
    struct wlan_network *pnetwork;
...
void _rtw_free_network(struct mlme_priv *pmlmepriv,
    ...
    if (pnetwork->fixed == _TRUE) goto exit;
...
struct wlan_network *rtw_alloc_network(
    struct mlme_priv *pmlmepriv) /* (_queue	*free_queue) */
{
    struct wlan_network *pnetwork;
...
int is_same_network(WLAN_BSSID_EX *src, WLAN_BSSID_EX *dst, u8 feature) {
    ...
    if (rtw_bug_check(dst, src, &s_cap, &d_cap) == _FALSE) return _FALSE;
    ...
    /* Wi-Fi driver doesn't consider the situation of BCN and ProbRsp sent from
     * the same hidden AP, it considers these two packets are sent from
     * different AP. Therefore, the scan queue may store two scan results of the
     * same hidden AP, likes below.
     *
     *  index            bssid              ch    RSSI   SdBm  Noise   age flag
     * ssid 1    00:e0:4c:55:50:01    153   -73     -73        0     7044
     * [WPS][ESS]     RTK5G 3    00:e0:4c:55:50:01    153   -73     -73        0
     * 7044   [WPS][ESS]
     *
     * Original rules will compare Ssid, SsidLength, MacAddress, s_cap, d_cap at
     * the same time. Wi-Fi driver will assume that the BCN and ProbRsp sent
     * from the same hidden AP are the same network after we add an additional
     * rule to compare SsidLength and Ssid. It means the scan queue will not
     * store two scan results of the same hidden AP, it only store ProbRsp. For
     * customer request.
     */
    ...
    if (((_rtw_memcmp(src->MacAddress, dst->MacAddress, ETH_ALEN)) == _TRUE) &&
        ...
        if ((src->Ssid.SsidLength == dst->Ssid.SsidLength) &&
            (((_rtw_memcmp(src->Ssid.Ssid, dst->Ssid.Ssid,
                           src->Ssid.SsidLength)) ==
              _TRUE) ||  // Case of normal AP
             (is_all_null(src->Ssid.Ssid, src->Ssid.SsidLength) == _TRUE ||
              is_all_null(dst->Ssid.Ssid, dst->Ssid.SsidLength) ==
                  _TRUE)))  // Case of hidden AP
            return _TRUE;
...
struct wlan_network *_rtw_find_same_network(_queue *scanned_queue,
    ...
    while (plist != phead) {
        ...
        if (is_same_network(&network->network, &found->network, 0)) break;

        plist = get_next(plist);
    }

    if (plist == phead) found = NULL;
...
struct wlan_network *rtw_find_same_network(_queue *scanned_queue,
    ...
    if (scanned_queue == NULL || network == NULL) goto exit;
...
void update_network(WLAN_BSSID_EX *dst, WLAN_BSSID_EX *src, _adapter *padapter,
    ...
    if (strcmp(dst->Ssid.Ssid, DBG_RX_SIGNAL_DISPLAY_SSID_MONITORED) == 0) {
        RTW_INFO(FUNC_ADPT_FMT " %s(" MAC_FMT
                               ", ch%u) ss_ori:%3u, sq_ori:%3u, rssi_ori:%3ld, "
                               "ss_smp:%3u, sq_smp:%3u, rssi_smp:%3ld\n",
                 FUNC_ADPT_ARG(padapter), src->Ssid.Ssid,
                 MAC_ARG(src->MacAddress), src->Configuration.DSConfig, ss_ori,
                 sq_ori, rssi_ori, ss_smp, sq_smp, rssi_smp);
    }
#endif
    ...
    if (strcmp(dst->Ssid.Ssid, DBG_RX_SIGNAL_DISPLAY_SSID_MONITORED) == 0) {
        RTW_INFO(FUNC_ADPT_FMT
                 ...
                 FUNC_ADPT_ARG(padapter), dst->Ssid.Ssid,
                 MAC_ARG(dst->MacAddress), dst->PhyInfo.SignalStrength,
                 dst->PhyInfo.SignalQuality, dst->Rssi);
    }
#endif

#if 0 /* old codes, may be useful one day...                                      \
       * 	RTW_INFO("update_network: rssi=0x%lx dst->Rssi=%d                       \
       * ,dst->Rssi=0x%lx ,                                                       \
       * src->Rssi=0x%lx",(dst->Rssi+src->Rssi)/2,dst->Rssi,dst->Rssi,src->Rssi); \
       */
...
bool rtw_update_scanned_network(_adapter *adapter, WLAN_BSSID_EX *target) {
    ...
    while (1) {
        ...
        if (rtw_roam_flags(adapter)) {
            /* TODO: don't  select netowrk in the same ess as choice if it's new
             * enough*/
        }
        if (pnetwork->fixed) {
            plist = get_next(plist);
        ...
        if ((choice == NULL) || (pnetwork->network.PhyInfo.SignalStrength <
                                 choice->network.PhyInfo.SignalStrength))
...
#else
        if (choice == NULL ||
            rtw_time_after(choice->last_scanned, pnetwork->last_scanned))
#ifdef CONFIG_RTW_MESH
            if (!MLME_IS_MESH(adapter) || !MLME_IS_ASOC(adapter) ||
                !rtw_bss_is_same_mbss(&pmlmepriv->cur_network.network,
                                      &pnetwork->network))
    ...
    if (!target_find) {
        if (_rtw_queue_empty(&(pmlmepriv->free_bss_pool)) == _TRUE) {
            ...
            pnetwork = choice;
            if (pnetwork == NULL) goto unlock_scan_queue;

#ifdef CONFIG_RSSI_PRIORITY
            RTW_DBG("%s => ssid:%s ,bssid:" MAC_FMT
                    "  will be deleted from scanned_queue (rssi:%ld , ss:%d)\n",
                    __func__, pnetwork->network.Ssid.Ssid,
                    MAC_ARG(pnetwork->network.MacAddress),
                    pnetwork->network.Rssi,
                    pnetwork->network.PhyInfo.SignalStrength);
#else
            RTW_DBG("%s => ssid:%s ,bssid:" MAC_FMT
                    " will be deleted from scanned_queue\n",
                    __func__, pnetwork->network.Ssid.Ssid,
                    MAC_ARG(pnetwork->network.MacAddress));
#endif
...
#endif
            _rtw_memcpy(&(pnetwork->network), target,
                        get_WLAN_BSSID_EX_sz(target));
            pnetwork->bcn_keys_valid = 0;
            if (target->Reserved[0] == BSS_TYPE_BCN ||
                target->Reserved[0] == BSS_TYPE_PROB_RSP)
 ...
 */
        ...
        if (target->Reserved[0] == BSS_TYPE_BCN ||
            ...
            if (target->InfrastructureMode == Ndis802_11Infrastructure) {
                ...
                if (!is_hidden_ssid_ap) /* update last time it's non hidden ssid
                                           AP */
        ...
        if (target->InfrastructureMode == Ndis802_11_mesh ||
            ...
            update_ie = _TRUE;
        else if (target->InfrastructureMode == Ndis802_11Infrastructure &&
                 !pnetwork->fixed &&
                 rtw_get_passing_time_ms(pnetwork->last_non_hidden_ssid_ap) >
                     SCANQUEUE_LIFETIME)
            update_ie = _TRUE;
            ...
            if (is_hidden_ssid(bcn_keys.ssid, bcn_keys.ssid_len)) {
                ...
                _rtw_memcpy(bcn_keys.ssid, pnetwork->bcn_keys.ssid,
                            pnetwork->bcn_keys.ssid_len);
        ...
        if (!MLME_IS_MESH(adapter) || !MLME_IS_ASOC(adapter) ||
            pnetwork->network.Configuration.DSConfig !=
                target->Configuration.DSConfig ||
            rtw_get_passing_time_ms(last_scanned) >
                adapter->mesh_cfg.peer_sel_policy.scanr_exp_ms ||
...
void rtw_add_network(_adapter *adapter, WLAN_BSSID_EX *pnetwork) {
    ...
    /* Wi-Fi driver will update the current network if the scan result of the
     * connected AP be updated by scan. */
    update_ie = rtw_update_scanned_network(adapter, pnetwork);

    if (update_ie) update_current_network(adapter, pnetwork);
...
void rtw_atimdone_event_callback(_adapter *adapter, u8 *pbuf) { return; }
...
static void process_80211d(PADAPTER padapter, WLAN_BSSID_EX *bssid) {
    ...
    if (pregistrypriv->enable80211d &&
        ...
        if (!chplan_sta) goto done_update_chplan_from_ap;
        ...
        if (pregistrypriv->wireless_mode & WIRELESS_11G) {
            do {
                if ((i == MAX_CHANNEL_NUM) || (chplan_sta[i].ChannelNum == 0) ||
                    (chplan_sta[i].ChannelNum > 14))
                    break;

                if ((j == chplan_ap.Len) || (chplan_ap.Channel[j] > 14)) break;
            ...
            while ((j < chplan_ap.Len) && (chplan_ap.Channel[j] <= 14)) j++;
        }
        ...
        if (pregistrypriv->wireless_mode & WIRELESS_11A) {
            do {
                ...
                if ((j == chplan_ap.Len) || (chplan_ap.Channel[j] == 0)) break;
...
void rtw_survey_event_callback(_adapter *adapter, u8 *pbuf) {
    ...
    if ((check_fwstate(pmlmepriv, _FW_UNDER_LINKING)) == _FALSE) {
        if (pnetwork->Ssid.Ssid[0] == 0) pnetwork->Ssid.SsidLength = 0;
...
void rtw_surveydone_event_callback(_adapter *adapter, u8 *pbuf) {
        ...
        if ((check_fwstate(pmlmepriv, WIFI_ADHOC_STATE) == _TRUE)) {
            if (check_fwstate(pmlmepriv, _FW_LINKED) == _FALSE) {
                ...
                if (rtw_select_and_join_from_scanned_queue(pmlmepriv) ==
                    ...
                    /* pmlmepriv->fw_state ^= _FW_UNDER_SURVEY; */ /* because
                                                                      don't set
                                                                      assoc_timer
                                                                    */
                    _clr_fwstate_(pmlmepriv, _FW_UNDER_SURVEY);

                    _rtw_memset(&pdev_network->Ssid, 0,
            ...
            if (_SUCCESS == s_ret)
                ...
                RTW_INFO(
                    "try_to_join, but select scanning queue fail, to_roam:%d\n",
                    rtw_to_roam(adapter));

                if (rtw_to_roam(adapter) != 0) {
                    struct sitesurvey_parm scan_parm;
                    u8 ssc_chk =
                        rtw_sitesurvey_condition_check(adapter, _FALSE);

                    rtw_init_sitesurvey_parm(adapter, &scan_parm);
                    _rtw_memcpy(&scan_parm.ssid[0], &pmlmepriv->assoc_ssid,
                                sizeof(NDIS_802_11_SSID));
...
#else
                    receive_disconnect(
                        adapter, pmlmepriv->cur_network.network.MacAddress,
    ...
    if (rtw_mesh_offch_candidate_accepted(adapter)) {
        ...
        if (ch && pmlmepriv->cur_network.network.Configuration.DSConfig != ch) {
            ...
            if (ifbmp) {
                ...
                rtw_change_bss_chbw_cmd(adapter, RTW_CMDF_DIRECTLY, ifbmp, 0,
                                        ch, REQ_BW_ORI, REQ_OFFSET_NONE);
                issue_probereq_ex(adapter,
                                  &pmlmepriv->cur_network.network.mesh_id, NULL,
                                  0, 0, 0, 0);
            } else
                rtw_warn_on(1);
        }
    ...
    if (parm->acs) {
        ...
        if (ifbmp)
            rtw_change_bss_chbw_cmd(adapter, RTW_CMDF_DIRECTLY, ifbmp, 0,
                                    REQ_CH_INT_INFO, REQ_BW_ORI,
                                    REQ_OFFSET_NONE);
    }
#endif
}

u8 _rtw_sitesurvey_condition_check(const char *caller, _adapter *adapter,
    ...
    if (registry_par->adaptivity_en && rtw_phydm_get_edcca_flag(adapter) &&
    ...
    if (check_fwstate(pmlmepriv, WIFI_AP_STATE)) {
        if (check_fwstate(pmlmepriv, WIFI_UNDER_WPS)) {
            ...
            RTW_INFO("%s (" ADPT_FMT
                     ") : scan abort!!AP mode under linking (fwstate=0x%x)\n",
            ...
            RTW_INFO("%s (" ADPT_FMT
                     ") : scan abort!!AP mode under survey (fwstate=0x%x)\n",
                     caller, ADPT_ARG(adapter), pmlmepriv->fw_state);
        ...
        if (check_fwstate(pmlmepriv, _FW_UNDER_LINKING) == _TRUE) {
            RTW_INFO("%s (" ADPT_FMT
                     ") : scan abort!!STA mode under linking (fwstate=0x%x)\n",
                     caller, ADPT_ARG(adapter), pmlmepriv->fw_state);
            ss_condition = SS_DENY_SELF_STA_UNDER_LINKING;
            goto _exit;
        } else if (check_fwstate(pmlmepriv, _FW_UNDER_SURVEY) == _TRUE) {
            RTW_INFO("%s (" ADPT_FMT
                     ") : scan abort!!STA mode under survey (fwstate=0x%x)\n",
                     caller, ADPT_ARG(adapter), pmlmepriv->fw_state);
    ...
    if (rtw_mi_buddy_check_fwstate(adapter,
                                   _FW_UNDER_LINKING | WIFI_UNDER_WPS)) {
        RTW_INFO("%s (" ADPT_FMT
                 ") : scan abort!! buddy_intf under linking or wps\n",
                 caller, ADPT_ARG(adapter));
     ...
     */
    if ((rtw_get_passing_time_ms(pmlmepriv->lastscantime) >
...
void rtw_free_assoc_resources(_adapter *adapter, u8 lock_scanned_queue) {
    ...
    if (check_fwstate(pmlmepriv, WIFI_STATION_STATE)) {
        ...
        psta =
            rtw_get_stainfo(&adapter->stapriv, tgt_network->network.MacAddress);
    ...
    if (check_fwstate(pmlmepriv, WIFI_UNDER_WPS) ||
        ...
        RTW_INFO(
            "Dont free disconnecting network of scanned_queue due to uner %s "
            "%s phase\n\n",
        ...
        if (pwlan) {
            ...
            RTW_ERR(
                "Free disconnecting network of scanned_queue failed due to "
                "pwlan == NULL\n\n");
    }
    ...
    if ((check_fwstate(pmlmepriv, WIFI_ADHOC_MASTER_STATE) &&
        ...
        if (pwlan) rtw_free_network_nolock(adapter, pwlan);
    }
...
void rtw_indicate_disconnect(_adapter *padapter, u16 reason,
    ...
    if (pmlmepriv->cur_network_scanned) {
        ...
        if (current_joined_bss) {
            ...
            if (wps_ie && wpsie_len > 0) {
                u8 *attr = NULL;
                u32 attr_len;
                attr = rtw_get_wps_attr(wps_ie, wpsie_len,
                                        WPS_ATTR_SELECTED_REGISTRAR, NULL,
                                        &attr_len);
                if (attr) *(attr + 4) = 0;
            }
        }
    ...
    if (rtw_to_roam(padapter) > 0) _clr_fwstate_(pmlmepriv, _FW_LINKED);
...
static u32 _rtw_wait_scan_done(_adapter *adapter, u8 abort, u32 timeout_ms) {
    ...
    while (check_fwstate(pmlmepriv, _FW_UNDER_SURVEY) &&
        ...
        if (RTW_CANNOT_RUN(adapter)) break;
...
static u32 _rtw_wait_join_done(_adapter *adapter, u8 abort, u32 timeout_ms) {
    ...
    if (abort) set_link_timer(pmlmeext, 1);
...
static struct sta_info *rtw_joinbss_update_stainfo(
    ...
    if (psta) { /* update ptarget_sta */
        ...
        psta->bssratelen =
            rtw_get_rateset_len(pnetwork->network.SupportedRates);
        ...
        if (padapter->securitypriv.dot11AuthAlgrthm == dot11AuthAlgrthm_8021X) {
            ...
            ie = rtw_get_ie(pnetwork->network.IEs + _BEACON_IE_OFFSET_,
                            WLAN_EID_RSN, &ie_len,
                            (pnetwork->network.IELength - _BEACON_IE_OFFSET_));
            if (ie && ie_len > 0 &&
                rtw_parse_wpa2_ie(ie, ie_len + 2, NULL, NULL, NULL, &mfp_opt) ==
                    _SUCCESS) {
            ...
            _rtw_memset((u8 *)&psta->dot118021x_UncstKey, 0,
                        sizeof(union Keytype));
            _rtw_memset((u8 *)&psta->dot11tkiprxmickey, 0,
                        sizeof(union Keytype));
            _rtw_memset((u8 *)&psta->dot11tkiptxmickey, 0,
                        sizeof(union Keytype));
        }
        ...
        for (i = 0; i < 16; i++) {
            ...
            rtw_clear_bit(RTW_RECV_ACK_OR_TIMEOUT,
                          &preorder_ctrl->rec_abba_rsp_ack);
#ifdef DBG_RX_SEQ
            RTW_INFO("DBG_RX_SEQ " FUNC_ADPT_FMT
                     " tid:%u SN_CLEAR indicate_seq:%u "
                     "preorder_ctrl->rec_abba_rsp_ack:%lu\n",
...
#endif
#ifdef CONFIG_RTS_FULL_BW
    rtw_parse_sta_vendor_ie_8812(padapter, psta,
                                 BSS_EX_TLV_IES(&cur_network->network),
                                 BSS_EX_TLV_IES_LEN(&cur_network->network));
#endif
    return psta;
}
 ...
 *"ptarget_sta" & "ptarget_wlan" exist. if join_res > 0, for
 *(fw_state==WIFI_ADHOC_STATE), we only check if "ptarget_wlan" exist. if
 *join_res > 0, update "cur_network->network" from "pnetwork->network" if
 *(ptarget_wlan !=NULL).
 *
 * Return
...
int rtw_joinbss_event_prehandle(_adapter *adapter, u8 *pbuf, u16 status) {
    ...
    if (pnetwork->network.Length > sizeof(WLAN_BSSID_EX)) goto exit;
        ...
        if (check_fwstate(pmlmepriv, _FW_UNDER_LINKING)) {
            ...
            if (check_fwstate(pmlmepriv, _FW_LINKED)) {
                if (the_same_macaddr == _TRUE)
                    ptarget_wlan =
                        _rtw_find_network(&pmlmepriv->scanned_queue,
                    ...
                    pcur_sta = rtw_get_stainfo(pstapriv,
                                               cur_network->network.MacAddress);
                    if (pcur_sta) {
                        /* _enter_critical_bh(&(pstapriv->sta_hash_lock),
                         * &irqL2); */
                        rtw_free_stainfo(adapter, pcur_sta);
                        /* _exit_critical_bh(&(pstapriv->sta_hash_lock),
                         * &irqL2); */
                    }
            ...
            if (MLME_IS_STA(adapter) || MLME_IS_ADHOC(adapter)) {
                ...
                if (err) {
                    RTW_ERR(FUNC_ADPT_FMT
                            ": Fail to indicate connect! err=%d\n",
...
void rtw_sta_media_status_rpt(_adapter *adapter, struct sta_info *sta,
    ...
    if (connected && !rtw_macid_is_bmc(macid_ctl, sta->cmn.mac_id)) {
        miracast_enabled =
            STA_OP_WFD_MODE(sta) != 0 && is_miracast_enabled(adapter);
        miracast_sink =
            miracast_enabled && (STA_OP_WFD_MODE(sta) & MIRACAST_SINK);
    ...
    rtw_hal_set_FwMediaStatusRpt_single_cmd(adapter, connected,
                                            miracast_enabled, miracast_sink,
                                            role, sta->cmn.mac_id);
}
...
u8 rtw_sta_media_status_rpt_cmd(_adapter *adapter, struct sta_info *sta,
    ...
    cmd_parm = (struct drvextra_cmd_parm *)rtw_zmalloc(
        sizeof(struct drvextra_cmd_parm));
    if (cmd_parm == NULL) {
    ...
    rpt_parm = (struct sta_media_status_rpt_cmd_parm *)rtw_zmalloc(
        sizeof(struct sta_media_status_rpt_cmd_parm));
    if (rpt_parm == NULL) {
        rtw_mfree((u8 *)cmdobj, sizeof(struct cmd_obj));
...
inline void rtw_sta_media_status_rpt_cmd_hdl(
    _adapter *adapter, struct sta_media_status_rpt_cmd_parm *parm) {
...
void rtw_stassoc_event_callback(_adapter *adapter, u8 *pbuf) {
    ...
    if (rtw_access_ctrl(adapter, pstassoc->macaddr) == _FALSE) return;
#endif
            ...
            if (!MLME_IS_MESH(adapter)) {
                ...
                if (psta->passoc_req && psta->assoc_req_len > 0) {
                    ...
                    if (passoc_req) {
                        ...
                        _rtw_memcpy(passoc_req, psta->passoc_req,
                                    assoc_req_len);
                ...
                if (passoc_req && assoc_req_len > 0) {
                    rtw_cfg80211_indicate_sta_assoc(adapter, passoc_req,
                                                    assoc_req_len);
    ...
    /* submit SetStaKey_cmd to tell fw, fw will allocate an CAM entry for this
     * sta	 */
...
void rtw_ft_info_init(struct ft_roam_info *pft) {
    ...
    pft->ft_flags = 0 | RTW_FT_EN | RTW_FT_OTD_EN
...
u8 rtw_ft_chk_roaming_candidate(_adapter *padapter,
    ...
    if (!_rtw_memcmp(&pft_roam->mdid, (pmdie + 2), 2)) return _FALSE;
...
void rtw_ft_update_stainfo(_adapter *padapter, WLAN_BSSID_EX *pnetwork) {
    ...
    if (psta == NULL) psta = rtw_alloc_stainfo(pstapriv, pnetwork->MacAddress);
...
void rtw_ft_reassoc_event_callback(_adapter *padapter, u8 *pbuf) {
    ...
    if (ft_evt_parms.target_ap)
        _rtw_memcpy((void *)ft_evt_parms.target_ap, pstassoc->macaddr,
                    ETH_ALEN);
    else
        goto err_1;
...
void rtw_roam_nb_info_init(_adapter *padapter) {
    ...
    rtw_init_timer(&pnb->roam_scan_timer, padapter, rtw_wnm_roam_scan_hdl,
                   padapter);
#endif
}
...
u8 rtw_roam_nb_scan_list_set(_adapter *padapter,
    ...
    if (!rtw_chk_roam_flags(padapter, RTW_ROAM_ACTIVE)) return ret;

    if (!pmlmepriv->need_to_roam) return ret;
...
void rtw_sta_mstatus_disc_rpt(_adapter *adapter, u8 mac_id) {
    ...
    if (mac_id < macid_ctl->num) {
        ...
        if (!id_is_shared) {
            rtw_hal_set_FwMediaStatusRpt_single_cmd(adapter, 0, 0, 0, 0,
                                                    mac_id);
            /*
...
void rtw_stadel_event_callback(_adapter *adapter, u8 *pbuf) {
    ...
    if (psta) rtw_wfd_st_switch(psta, 0);
    ...
    if (check_fwstate(pmlmepriv, WIFI_STATION_STATE)) {
        ...
        if (roam == _TRUE) {
            if (rtw_to_roam(adapter) > 0)
                rtw_dec_to_roam(adapter); /* this stadel_event is caused by
                                             roaming, decrease to_roam */
            else if (rtw_to_roam(adapter) == 0)
                rtw_set_to_roam(adapter,
...
void rtw_join_timeout_handler(void *ctx) {
    ...
    if (RTW_CANNOT_RUN(adapter)) return;
...
#endif
                rtw_indicate_disconnect(adapter, pmlmepriv->join_status,
                                        _FALSE);
                break;
            }
        }
...
#endif
    {
         ...
         * != FW_LINKED */
        rtw_cfg80211_indicate_disconnect(adapter, pmlmepriv->join_status,
                                         _FALSE);
#endif /* CONFIG_IOCTL_CFG80211 */
    }
...
void rtw_mlme_reset_auto_scan_int(_adapter *adapter, u8 *reason) {
...
#endif
    u8 u_ch;
    u32 interval_ms = 0xffffffff; /* 0xffffffff: special value to make min()
                                     works well, also means no auto scan */
    ...
    *reason = RTW_AUTO_SCAN_REASON_UNSPECIFIED;
    ...
    if (hal_chk_bw_cap(adapter, BW_CAP_40M) &&
        is_client_associated_to_ap(adapter) == _TRUE && u_ch >= 1 &&
        u_ch <= 14 && adapter->registrypriv.wifi_spec
    ...
    if (adapter->mesh_cfg.peer_sel_policy.offch_find_int_ms &&
        ...
        && (!rfctl->radar_detect_ch ||
            (IS_CH_WAITING(rfctl) && !IS_UNDER_CAC(rfctl)))
#endif
    ) {
        interval_ms = rtw_min(
            interval_ms, adapter->mesh_cfg.peer_sel_policy.offch_find_int_ms);
        *reason |= RTW_AUTO_SCAN_REASON_MESH_OFFCH_CAND;
    }
#endif
#endif /* CONFIG_RTW_MESH */

    if (interval_ms == 0xffffffff) interval_ms = 0;
...
void rtw_drv_scan_by_self(_adapter *padapter, u8 reason) {
    ...
    if (!rtw_is_adapter_up(padapter)) goto exit;
#else
    if (rtw_is_scan_deny(padapter)) goto exit;
...
#ifdef CONFIG_P2P
    if (!rtw_p2p_chk_state(&padapter->wdinfo, P2P_STATE_NONE)) goto exit;
#endif

#ifdef CONFIG_TDLS
    if (padapter->tdlsinfo.link_established == _TRUE) goto exit;
#endif
...
void rtw_iface_dynamic_check_timer_handlder(_adapter *adapter) {
    ...
    if (is_drv_in_lps(adapter)) {
        ...
        if (bEnterPS) {
            ...
            /* call rtw_lps_ctrl_wk_cmd(padapter, LPS_CTRL_LEAVE, 0) in
...
static void collect_sta_traffic_statistics(_adapter *adapter) {
    ...
    for (i = 0; i < MACID_NUM_SW_LIMIT; i++) {
        ...
        if (sta && !is_broadcast_mac_addr(sta->cmn.mac_addr)) {
            ...
            curr_tx_bytes =
                sta->sta_stats.tx_bytes - sta->sta_stats.last_tx_bytes;
            curr_rx_bytes =
                sta->sta_stats.rx_bytes - sta->sta_stats.last_rx_bytes;
            ...
            sta->sta_stats.smooth_tx_tp_kbits =
                (sta->sta_stats.smooth_tx_tp_kbits * 6 / 10) +
                (sta->sta_stats.tx_tp_kbits * 4 / 10); /*Kbps*/
            sta->sta_stats.smooth_rx_tp_kbits =
                (sta->sta_stats.smooth_rx_tp_kbits * 6 / 10) +
                (sta->sta_stats.rx_tp_kbits * 4 / 10); /*Kbps*/

            curr_tx_mbytes = (curr_tx_bytes / 2) >> 20; /*MBps*/
            curr_rx_mbytes = (curr_rx_bytes / 2) >> 20; /*MBps*/

            sta->cmn.tx_moving_average_tp =
                (sta->cmn.tx_moving_average_tp / 10) +
                (curr_tx_mbytes * 9 / 10); /*MBps*/

            sta->cmn.rx_moving_average_tp =
                (sta->cmn.rx_moving_average_tp / 10) +
                (curr_rx_mbytes * 9 / 10); /*MBps*/
...
static void collect_traffic_statistics(_adapter *padapter) {
    ...
    /*_rtw_memset(&pdvobjpriv->traffic_stat, 0, sizeof(struct
     * rtw_traffic_statistics));*/
...
void rtw_dynamic_check_timer_handlder(void *ctx) {
    ...
    if (!rtw_is_hw_init_completed(adapter)) goto exit;

    if (RTW_CANNOT_RUN(adapter)) goto exit;

    collect_traffic_statistics(adapter);
    collect_sta_traffic_statistics(adapter);
    rtw_mi_dynamic_check_timer_handlder(adapter);

    if (!is_drv_in_lps(adapter)) rtw_dynamic_chk_wk_cmd(adapter);
...
static int rtw_check_roaming_candidate(struct mlme_priv *mlme,
    ...
    if (rtw_chset_search_ch(chset, ch) < 0) goto exit;
    ...
    if (rtw_rson_isupdate_roamcan(mlme, candidate, competitor)) goto update;
    goto exit;
#endif
    ...
    if (rtw_is_desired_network(adapter, competitor) == _FALSE) goto exit;

#ifdef CONFIG_LAYER2_ROAMING
    if (mlme->need_to_roam == _FALSE) goto exit;
#endif
...
int rtw_select_roaming_candidate(struct mlme_priv *mlme) {
    ...
    if (candidate == NULL) {
        /*	if parent note lost the path to root and there is no other
         * cadidate, report disconnection	*/
#if defined(CONFIG_RTW_REPEATER_SON) && (!defined(CONFIG_RTW_REPEATER_SON_ROOT))
        ...
        rson_score = rtw_cal_rson_score(
            &rson_curr, mlme->cur_network_scanned->network.Rssi);
        if (check_fwstate(mlme, _FW_LINKED) &&
            ...
            receive_disconnect(adapter,
                               mlme->cur_network_scanned->network.MacAddress,
        ...
        RTW_INFO("%s: candidate: %s(" MAC_FMT ", ch:%u) rson_score:%d\n",
                 __FUNCTION__, candidate->network.Ssid.Ssid,
...
static int rtw_check_join_candidate(struct mlme_priv *mlme,
    ...
    if (rtw_rson_choose(candidate, competitor)) {
        *candidate = competitor;
        ...
        RTW_INFO("[assoc_ssid:%s] new candidate: %s(" MAC_FMT
                 ", ch%u) rson_score:%d\n",
                 mlme->assoc_ssid.Ssid, (*candidate)->network.Ssid.Ssid,
                 MAC_ARG((*candidate)->network.MacAddress),
                 (*candidate)->network.Configuration.DSConfig, rson_score);
        return _TRUE;
    }
    ...
    if (updated) {
        RTW_INFO(
            "[by_bssid:%u][assoc_ssid:%s][to_roam:%u] "
            "new candidate: %s(" MAC_FMT ", ch%u) rssi:%d\n",
            mlme->assoc_by_bssid, mlme->assoc_ssid.Ssid, rtw_to_roam(adapter),
            (*candidate)->network.Ssid.Ssid,
            MAC_ARG((*candidate)->network.MacAddress),
            (*candidate)->network.Configuration.DSConfig,
...
int rtw_select_and_join_from_scanned_queue(struct mlme_priv *pmlmepriv) {
    ...
    while (!rtw_end_of_queue_search(phead, pmlmepriv->pscanned)) {
        pnetwork =
            LIST_CONTAINOR(pmlmepriv->pscanned, struct wlan_network, list);
        if (pnetwork == NULL) {
            ret = _FAIL;
            goto exit;
        }
...
candidate_exist:
...
#if 0 /* for WPA/WPA2 authentication, wpa_supplicant will expect \
         authentication from AP, it is needed to reconnect AP... */
    ...
    if (_TRUE == bSupportAntDiv) {
        ...
        RTW_INFO("#### Opt_Ant_(%s) , cur_Ant(%s)\n",
                 (MAIN_ANT == candidate->network.PhyInfo.Optimum_antenna)
                     ? "MAIN_ANT"
                     : "AUX_ANT",
 ...
 * This function is used for setting default uapsd max sp length to
 * uapsd_max_sp_len in qos_priv data structure from registry. In additional, it
 * will also map default uapsd ac to each uapsd TID, delivery-enabled and
 * trigger-enabled of corresponding TID.
 *
 * Arguments:
...
void rtw_uapsd_use_default_setting(_adapter *padapter) {
    ...
    if (pregistrypriv->uapsd_ac_enable != 0) {
        ...
        /* check the uapsd setting of AC_VO from registry then map these setting
         * to each TID if necessary  */
        ...
        /* check the uapsd setting of AC_VI from registry then map these setting
         * to each TID if necessary  */
        ...
        /* check the uapsd setting of AC_BK from registry then map these setting
         * to each TID if necessary  */
        ...
        /* check the uapsd setting of AC_BE from registry then map these setting
         * to each TID if necessary  */
        ...
        RTW_INFO(
            "[WMMPS] UAPSD MAX SP Len = 0x%02x, UAPSD TID enabled = 0x%02x\n",
...
/*
 * rtw_is_wmmps_mode
 * This function is used for checking whether Driver and an AP support uapsd
 * function or not. If both of them support uapsd function, it will return true.
 ...
 *SecIsInPreAuthKeyList(), 2006-10-13.) Added by Annie, 2006-05-07.
 *
 * Search by BSSID,
 * Return Value:
 *		-1		:if there is no pre-auth key in the  table
 *		>=0		:if there is pre-auth key, and   return the
 *entry id
 *
 *   */
...
int rtw_rsn_sync_pmkid(_adapter *adapter, u8 *ie, uint ie_len, int i_ent) {
    ...
    if (i_ent < 0 && info.pmkid_cnt == 0) goto exit;
    ...
    if (info.gmcs) _rtw_memcpy(gm_cs, info.gmcs, 4);
    ...
    if (info.gmcs) _rtw_memcpy(info.pmkid_list + 16 * info.pmkid_cnt, gm_cs, 4);
...
sint rtw_restruct_sec_ie(_adapter *adapter, u8 *out_ie) {
    ...
    if (check_fwstate(pmlmepriv, WIFI_UNDER_WPS)) {
        ...
        psecuritypriv->supplicant_ie[1]+2); for(jj=0; jj <
        psecuritypriv->supplicant_ie[1]+2; jj++) printk(" %02x ",
        psecuritypriv->supplicant_ie[jj]); printk("\n");
        }*/
...
void rtw_ht_use_default_setting(_adapter *padapter) {
    ...
    if (MLME_IS_STA(padapter) && !MLME_IS_GC(padapter) &&
        !MLME_IS_ADHOC(padapter) && !MLME_IS_MESH(padapter))
#endif
    {
        ...
        if (TEST_FLAG(pregistrypriv->beamform_cap, BIT4) &&
            bHwSupportBeamformer) {
            SET_FLAG(phtpriv->beamform_cap, BEAMFORMING_HT_BEAMFORMER_ENABLE);
            RTW_INFO("[HT] HAL Support Beamformer\n");
        }
        if (TEST_FLAG(pregistrypriv->beamform_cap, BIT5) &&
...
unsigned int rtw_restructure_ht_ie(_adapter *padapter, u8 *in_ie, u8 *out_ie,
    ...
    if (phtpriv->sgi_20m) ht_capie.cap_info |= IEEE80211_HT_CAP_SGI_20;
    ...
    if (cbw40_enable) {
        ...
        if (in_ie == NULL) {
            ...
            if (check_fwstate(pmlmepriv, WIFI_STATION_STATE)) {
                ...
                if (oper_bw > CHANNEL_WIDTH_40) oper_bw = CHANNEL_WIDTH_40;
            } else
                /* TDLS: TODO 40? */
            ...
            if (oper_bw == CHANNEL_WIDTH_40) {
                ...
                if (p && ielen == HT_CAP_IE_LEN) {
                    oper_bw = GET_HT_CAP_ELE_CHL_WIDTH(p + 2)
                                  ? CHANNEL_WIDTH_40
                                  : CHANNEL_WIDTH_20;
        ...
        if (oper_bw == CHANNEL_WIDTH_40 &&
            oper_offset !=
                HAL_PRIME_CHNL_OFFSET_DONT_CARE /* check this because TDLS has
                                                   no info to set offset */
            && (!rtw_chset_is_chbw_valid(chset, channel, oper_bw, oper_offset,
                                         1, 1) ||
                (IS_DFS_SLAVE_WITH_RD(rfctl) &&
                 !rtw_rfctl_dfs_domain_unknown(rfctl) &&
                 rtw_chset_is_chbw_non_ocp(chset, channel, oper_bw,
                                           oper_offset)))) {
            oper_bw = CHANNEL_WIDTH_20;
            oper_offset = HAL_PRIME_CHNL_OFFSET_DONT_CARE;
            rtw_warn_on(!rtw_chset_is_chbw_valid(chset, channel, oper_bw,
                                                 oper_offset, 1, 1));
            if (IS_DFS_SLAVE_WITH_RD(rfctl) &&
                !rtw_rfctl_dfs_domain_unknown(rfctl))
                rtw_warn_on(rtw_chset_is_chbw_non_ocp(chset, channel, oper_bw,
                                                      oper_offset));
        }

        if (oper_bw == CHANNEL_WIDTH_40) {
            ht_capie.cap_info |= IEEE80211_HT_CAP_SUP_WIDTH;
            if (phtpriv->sgi_40m) ht_capie.cap_info |= IEEE80211_HT_CAP_SGI_40;
        }
    ...
    if (TEST_FLAG(phtpriv->stbc_cap, STBC_HT_ENABLE_RX)) {
        if ((pregistrypriv->rx_stbc == 0x3) || /* enable for 2.4/5 GHz */
             ...
             */
            rtw_hal_get_def_var(padapter, HAL_DEF_RX_STBC,
                                (u8 *)(&rx_stbc_nss));
    ...
    switch (rx_nss) {
        ...
        case 2:
            ...
            if (cbw40_enable && pregistrypriv->wifi_spec != 1)
                set_mcs_rate_by_mask(ht_capie.supp_mcs_set,
    ...
    */
    ...
    if (padapter->driver_rx_ampdu_spacing != 0xFF)
        ...
        if (padapter->securitypriv.dot11PrivacyAlgrthm == _AES_) {
            /*
             *	Todo : Each chip must to ask DD , this chip best ampdu_density
             *setting By yiwei.sun
             */
    ...
    if (in_ie != NULL) {
        ...
        if (p && (ielen == sizeof(struct ieee80211_ht_addt_info))) {
            out_len = *pout_len;
            pframe = rtw_set_ie(out_ie + out_len, _HT_ADD_INFO_IE_, ielen,
                                p + 2, pout_len);
        }
    }
...
void rtw_update_ht_cap(_adapter *padapter, u8 *pie, uint ie_len, u8 channel) {
    ...
    if ((!pmlmeinfo->HT_info_enable) || (!pmlmeinfo->HT_caps_enable)) return;
    ...
    if ((cbw40_enable) &&
        ...
        switch (tx_nss) {
            case 1:
                set_mcs_rate_by_mask(
                    pmlmeinfo->HT_caps.u.HT_cap_element.MCS_rate, MCS_RATE_1R);
                break;
            case 2:
#ifdef CONFIG_DISABLE_MCS13TO15
            ...
            case 3:
                set_mcs_rate_by_mask(
                    pmlmeinfo->HT_caps.u.HT_cap_element.MCS_rate, MCS_RATE_3R);
                break;
            case 4:
                set_mcs_rate_by_mask(
                    pmlmeinfo->HT_caps.u.HT_cap_element.MCS_rate, MCS_RATE_4R);
                break;
            default:
        ...
        switch ((pmlmeinfo->HT_info.infos[0] & 0x3)) {
            ...
            default:
                ...
                RTW_INFO(
                    "%s : ch offset is not assigned for HT40 mod , update "
                    "cur_bwmode=%u, cur_ch_offset=%u\n",
...
void rtw_append_exented_cap(_adapter *padapter, u8 *out_ie, uint *pout_len) {
    ...
    /*
       ...
       capabilities defined in the Extended Capabilities element, then the STA
       is not required to transmit the Extended Capabilities element.
    */
    if (_FALSE == _rtw_memcmp(cap_content, null_content, 8))
        pframe = rtw_set_ie(out_ie + *pout_len, EID_EXTCapability, 8,
...
inline void rtw_set_to_roam(_adapter *adapter, u8 to_roam) {
    if (to_roam == 0) adapter->mlmepriv.to_join = _FALSE;
...
sint rtw_linked_check(_adapter *padapter) {
    if (MLME_IS_AP(padapter) || MLME_IS_MESH(padapter) ||
        ...
        if (padapter->stapriv.asoc_sta_count > 2) return _TRUE;
    } else {
        /* Station mode */
...
u8 rtw_is_adapter_up(_adapter *padapter) {
    if (padapter == NULL) return _FALSE;

    if (RTW_CANNOT_RUN(padapter)) {
#ifdef DBG_ADAPTER_STATE_CHK
        RTW_INFO(FUNC_ADPT_FMT
                 " FALSE -bDriverStopped(%s) bSurpriseRemoved(%s)\n",
                 FUNC_ADPT_ARG(padapter),
...
#ifdef CONFIG_WFD
    if (on)
        rtw_st_ctl_register(&sta->st_ctl, SESSION_TRACKER_REG_ID_WFD,
                            &wfd_st_reg);
    else
